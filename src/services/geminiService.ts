...
ВАЖНО: В самом тексте отчёта НЕ создавай никакой раздел с CSV строкой и НЕ выводи заголовок с JSON метаданными. Для передачи служебных данных автозаполнения (если применимо) помести исключительно в самый конец ответа технический JSON блок:
\`\`\`json
{
  "extractedMeta": {
...
Выдавай отчёт профессиональным, структурированным, доказательным языком на русском языке.`;
  ...
  const cleanedReport = cleanMarkdownReport(reportText);
